...
from figures import performance_size_plot, radar_chart
from table import create_tables
from text_segments import ACKNOWLEDGEMENT, FAQ, CITATIONS
from leaderboard.meta import get_model_metas
from benchmark.models import BenchmarksData
from leaderboard.load_data import ALL_BENCHMARKS_DATA
from benchmark.benchmark import Benchmark, get_benchmarks

TASK_DOMAIN = Literal[
    "general"
]

TASK_TYPE = Literal["translation", "other"]
...
def update_citation(benchmark_name: str) -> str:
    citation_raw = CITATIONS.get(benchmark_name, 
                                                  """@inproceedings{translation2025,
...
def update_description(
    ...
    n_languages = len(benchmark.results)
    n_task_types = len(task_types)
    n_tasks = len(benchmark.tasks)
    
    domains_set = set()
    for task in benchmark.tasks:
        domains_set.update(task.domains)
...
def update_task_info(task_names: list[str]) -> gr.DataFrame:
            ...
            for model in result.models:
                for task in model.task_type:
                    ...
                    task_data.append({
                        "name": task.name,
                        "type": task.type,
                        "languages": task.source_languages,
                        "domains": task.domains,
                        "reference": model.reference or "",  # protection si None
                        #"main_score": task.main_score,
                        "modalities": task.modalities,
                    })

    if not task_data:
        return gr.DataFrame(pd.DataFrame(columns=[
            "Task Name", "Task Type", "Languages", "Domains", "Metric", "Modality"
        ]))

    df = pd.DataFrame(task_data)

    def format_list(x):
        return ", ".join(sorted(set(x))) if isinstance(x, list) else x
...
def load_results():
    ...
    for benchmark in benchmarks:
        results = benchmark.load_results(ALL_BENCHMARKS_DATA)
        scores = results.get_scores(format="long")
...
def get_leaderboard_app() -> gr.Blocks:
    ...
    benchmarks = sorted(get_benchmarks(), key=lambda x: x.name)
    ...
    default_benchmark = Benchmark(DEFAULT_BENCHMARK_NAME)
    default_results = all_benchmark_results[default_benchmark.name.lower()]
    ...
    with gr.Blocks(
        ...
        theme=gr.themes.Soft(
            font=[gr.themes.GoogleFont("Oswald"), "Arial", "sans-serif"]
        ),
        head=head,
    ) as demo:
        with gr.Sidebar(
        ...
        gr.Markdown(
            """
        ## Multilingual LLMs Leaderboard

        This leaderboard compares 100+ LLMs across 6000+ languages. For details on metrics, languages, tasks, and task types, please refer to the README or FAQ. Anyone is welcome [to add a model](https://github.com/judeLuther/EvaluLLM/blob/main/README.md), [add benchmarks](https://github.com/judeLuther/EvaluLLM/blob/main/README.md)  or [propose other changes to the leaderboard](https://github.com/judeLuther/EvaluLLM/blob/main/README.md).
        """
        )
        gr.Markdown(
        ...
        def on_benchmark_select(benchmark_name):
            ...
            benchmark = Benchmark(benchmark_name)
            languages = [task.source_languages for task in benchmark.tasks if task.source_languages]            
            ...
            types = {
                task.type for task in benchmark.tasks if task.type
            }
            modalities = set()
            for task in benchmark.tasks:
                modalities.update(task.modalities)
            ...
            benchmark_results = all_benchmark_results[benchmark_name.lower()]
            ...
            return (
                ...
                types,
                modalities,
                sorted([task.name for task in benchmark.tasks]),
                scores,
            )
        ...
        def update_task_list(
            ...
            start_time = time.time()
            tasks_to_keep = []
            for task in Benchmark(benchmark_name).tasks:
                if task.type not in type_select:
                    continue
                if task.domains is not None and not (
                    set(task.domains) & set(domain_select)
                ):
                    continue
                if task.source_languages is not None and not (
                    set(task.source_languages) & set(lang_select)
                ):
                    continue
                if task.modalities and not (
                    set(task.modalities) & set(modality_select)
                ):
                    continue
                tasks_to_keep.append(task.name)
            elapsed = time.time() - start_time
        ...
        def update_tables(
            ...
            tasks = set(tasks)
            benchmark = Benchmark(benchmark_name)
            benchmark_tasks = {task.name for task in benchmark.tasks}
