...
from benchmark.benchmark import get_benchmarks
...
STATIC_RESULTS = load_results()


class ModelMeta:
    def __init__(self, name, reference, max_tokens, embed_dim, n_parameters, memory_usage_mb, zero_shot_val):
        self.name = name
        ...
        self.zero_shot_val = zero_shot_val

    def zero_shot_percentage(self, tasks):
        return self.zero_shot_val


class Task:
    def __init__(self, name, task_type):
        self.metadata = type("Meta", (), {"type": task_type})

def get_model_meta(model_name):
    for benchmark in ALL_BENCHMARKS_DATA.benchmarks:
        for result in benchmark.results:
            for model in result.models:
                full_model_name = f"{model.model_name}_{result.language}"
                if full_model_name == model_name:
                    return ModelMeta(
                        name=full_model_name,
                        reference=model.reference or "https://example.com/fallback",
                        max_tokens=model.max_tokens,
                        embed_dim=model.embed_dim,
                        n_parameters=model.n_parameters,
                        memory_usage_mb=None, 
                        zero_shot_val=model.zero_shot_percentage
                    )

def get_task(task_name):
    task_type_map = defaultdict(lambda: "unknown")
    task_type_map.update({
        "translation": "translation",
        "other": "other"
    })

    return Task(task_name, task_type_map[task_name])


def get_tasks(tasks=None):
    all_tasks = {r["task_name"] for r in STATIC_RESULTS}
    selected_tasks = tasks or sorted(all_tasks)
    return [get_task(task) for task in selected_tasks]
